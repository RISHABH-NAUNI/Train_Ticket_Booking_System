...
def is_strong_password(password):
    ...
    return re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]{8,}$', password)
...
def get_destinations():
    ...
    trains = Train.query.filter_by(source=source).all()
    destinations = sorted(set(t.destination for t in trains if t.destination != source))
